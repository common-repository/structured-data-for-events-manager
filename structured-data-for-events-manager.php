...
Text Domain: structured-data-for-events-manager
...
{
	load_plugin_textdomain( 'structured-data-for-events-manager', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
} );
...
{    
    if ( !is_plugin_active( 'events-manager/events-manager.php' ) )
    {
        ...
        add_action( 'admin_notices', function ()
        {
            ...
            $message = sprintf( wp_kses( __( 'The <strong>Structured data for Events Manager</strong> plugin has been deactivated. Its activation depends on whether the <a href="%1$s" class="%2$s">Events Manager</a> plugin is active.', 'structured-data-for-events-manager' ), array(  'a' => array( 'href' => array(), 'class' => array() ), 'strong' => array() ) ), esc_url( get_admin_url(null, 'plugin-install.php?tab=plugin-information&amp;plugin=events-manager&amp;TB_iframe=true&amp;width=600&amp;height=550') ), esc_attr( 'thickbox open-plugin-details-modal' ) );
...
add_action( 'wp_footer', function () {
    ...
    si se ha marcado el check "Todo el día" las horas no se mostrarán
    ...
    // Fecha de inicio
    $ems_jsonld .= '"startDate": "' . get_post_meta( get_the_ID(), '_event_start_date', true ) . ( ($ems_todoeldia) ? '' : 'T' . get_post_meta( get_the_ID(), '_event_start_time', true ) ) . '",
    ';
    ...
    // Fecha de fin
    $ems_jsonld .= '"endDate": "' . get_post_meta( get_the_ID(), '_event_end_date', true ) . ( ($ems_todoeldia) ? '' : 'T' . get_post_meta( get_the_ID(), '_event_end_time', true ) ) . '"
    ';
    
    
    // Si se han habilitado las ubicaciones obtendremos sus datos
    // Es necesario el campo "location" para que la Herramienta de pruebas de datos estructurados
    // de Google no muestre error.
